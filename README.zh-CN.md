...
这是社区预览版，尚未完成完整的公开发布验收。已在非官方构建中使用固定版本的 Step Code runtime 执行 `step login`，并成功完成 Step Plan 浏览器登录。付费模型任务与真实工具审批流程，以及干净 Windows 环境中的安装、升级和卸载仍需验证。MCP 状态目前反映配置是否启用，不代表连接健康检查；部分高级扩展界面也尚未覆盖。
